...
def helper_moderate_content(text_to_moderate, client):
    try:
        ...
        report = {
            # flagged：若有任何政策被违反，则为简单的True/False值
            "flagged": mod_result.flagged,
            # categories：每个特定伤害类别的布尔值字典，例如仇恨或暴力
            "categories": dict(mod_result.categories),
            # scores：每个类别的原始置信度分数字典
            "scores": dict(mod_result.category_scores),
        }

        if report["flagged"]:
            logging.warning(
                f"Content was FLAGGED by moderation API. Report: {report['categories']}"
            )
        else:
