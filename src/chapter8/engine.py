...
def planner(goal, capabilities, client, generation_model):
    ...
    try:
        plan_json_string = helpers.call_llm_robust(
            system_prompt,
            goal,
            client=client,
...
def context_engine(
    ...
    for step in plan:
        ...
        try:
            ...
            mcp_resolved_input = helpers.create_mcp_message("Engine", resolved_input)
...
def execute_and_display(goal, config, client, pc, moderation_active):
    ...
    if moderation_active:
        ...
        moderation_report = helpers.helper_moderate_content(
            text_to_moderate=goal, client=client
        )
